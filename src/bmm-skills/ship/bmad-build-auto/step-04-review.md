...
5. **Review-only handoff** (`{review_only}` = `true`): if any surviving entry is `bad_spec` or `intent_gap`, stop before the repair loop below. Preserve the current code and existing commits; do not revert, amend implementation requirements, or invoke step-03 or an implementation subagent. Keep every finding in the triage log. Under `## Auto Run Result`, record the unresolved findings and their evidence, the required development or intent decision, and the verification needed after repair. HALT with status `blocked` and blocking condition `review requires development` for `bad_spec`, or `intent gap` when any `intent_gap` exists. Do not relabel required story work as `defer` or a follow-up recommendation to reach `done`. This is a handoff to the caller, not an automatic development dispatch.
6. Process entries in cascading order. If intent_gap exists, lower entries are moot; follow the intent_gap branch below. If bad_spec exists, lower entries are moot since code will be re-derived. If neither exists, process patch and defer normally. Before each bad_spec loopback, read `{spec_file}` frontmatter `review_loop_iteration` (missing means `0`), increment it by 1, and write it back. If it exceeds 5, append the triage-log entry for this pass, then HALT with status `blocked` and blocking condition `review repair loop exceeded 5 iterations (non-convergence)`.
...
Before finalizing, confirm no known mandatory implementation, acceptance, or verification work remains unresolved. If any remains, record it under `## Auto Run Result` and HALT with status `blocked` and the specific blocking condition. A deferred entry or follow-up recommendation does not make that work complete.
